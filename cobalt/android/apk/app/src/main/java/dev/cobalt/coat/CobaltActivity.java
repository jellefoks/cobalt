...
public abstract class CobaltActivity extends Activity {
  ...
  protected void createContent(final Bundle savedInstanceState) {
    ...
    AppEventBridge.handleStartEvent(mStartDeepLink);
  ...
  protected void onStop() {
    ...
    if (getJavaSwitches().containsKey(JavaSwitches.DELAY_FREEZE_ON_BACKGROUND)) {
      ...
      // Fire freeze event immediately upon entering stopped state in background
      AppEventBridge.handleLifecycleEvent(StarboardBridge.kSbEventTypeFreeze);
    }
  ...
  protected void onNewIntent(Intent intent) {
    ...
    getStarboardBridge().handleDeepLink(url);
  }
